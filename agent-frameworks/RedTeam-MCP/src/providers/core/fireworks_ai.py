...
class FireworksAIProvider(BaseProvider):
    ...
    def get_api_key(self, model_id: str = None) -> Optional[str]:
        ...
        api_key = os.getenv(env_var_name) or config.get('models.fireworks_ai_key')
        if not api_key or api_key == 'your_fireworks_ai_key_here':
            return None
        return api_key

    def get_model_string(self, model_id: str) -> str:
        return f"fireworks-ai/{model_id}"