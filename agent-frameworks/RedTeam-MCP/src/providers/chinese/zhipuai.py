...
class ZhipuaiProvider(BaseProvider):
    """Zhipuai provider implementation - OpenAI-compatible API"""

    def __init__(self):
        super().__init__('zhipuai')

    def get_api_key(self, model_id: str = None) -> Optional[str]:
        env_var_name = 'ZHIPU_API_KEY'
        api_key = os.getenv(env_var_name) or config.get('models.zhipuai_key')
