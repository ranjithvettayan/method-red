...
class RequestyProvider(BaseProvider):
    """Requesty provider implementation - OpenAI-compatible API"""

    def __init__(self):
        super().__init__('requesty')

    def get_api_key(self, model_id: str = None) -> Optional[str]:
        env_var_name = 'REQUESTY_API_KEY'
        api_key = os.getenv(env_var_name) or config.get('models.requesty_key')
        if not api_key or api_key == 'your_requesty_key_here':
            return None
        return api_key
