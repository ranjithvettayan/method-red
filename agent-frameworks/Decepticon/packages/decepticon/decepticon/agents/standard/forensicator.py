...
``packages/decepticon/decepticon/skills/standard/dfir/`` but no agent
consumed them. This file adds the missing dispatch surface so the
orchestrator can validate the offensive narrative from the defender's
side: confirm which TTPs left artifacts, extract IOCs, and build the
incident timeline that the report's detection-engineering section
depends on.
...
Tool surface (all via bash for the OSS bootstrap):
  ...
  - tshark / zeek for network forensics.
  - regripper / evtx_dump for Windows artifacts.

This agent is read / analysis oriented: it inspects evidence captured
in the engagement workspace, it does not attack.
"""
...
_SKILL_SOURCES: list[str] = ["/skills/standard/dfir/", "/skills/shared/"]


def create_forensicator_agent(
    *,
    backend: Any = None,
    llm: Any = None,
...
if is_bundle_enabled("standard"):
    graph = create_forensicator_agent()


SUBAGENT_SPEC = SubAgentSpec(
    name="forensicator",
    description=(
        "DFIR / forensics specialist for post-incident and purple-team "
        "validation. Use to confirm which TTPs left artifacts and to "
        "build the detection narrative: disk / memory / log / network "
        "timeline analysis, IOC extraction, artifact triage (Volatility, "
        "registry, event logs, PCAP). Analysis-only. Existing skill tree "
        "at skills/standard/dfir/."
    ),
    factory=create_forensicator_agent,
